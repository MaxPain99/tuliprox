...
grep -q 'Never mutate active_user_streams' "${TREE}/frontend/src/hooks/use_server_status.rs"
grep -q 'stream_display_key' "${TREE}/frontend/src/app/components/dashboard/stream_display/mod.rs"
grep -q 'Include preserved Catchup/HLS/DASH session rows' "${TREE}/backend/src/api/model/active_user_manager.rs"
grep -q 'Catchup-token Live/.ts segment sockets must preserve' "${TREE}/backend/src/api/model/active_user_manager.rs"
grep -q 'Active archive rows must not TTL-blink' "${TREE}/frontend/src/app/components/dashboard/stream_display/helpers.rs"
